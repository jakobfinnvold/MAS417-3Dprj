...
# https://www.makerhacks.com/modify-stl-cad-files-for-cnc-or-3d-print-using-python-programming/
# https://github.com/SolidCode/SolidPython/tree/master/solid
class STL:

    def __init__(self, weathersymbol, windspeed):
    ...
    def symbol_mesh(self):
        ...
        weather_symbol = f'./png/{self.weathersymbol}.png' # Using the line that was read from the txt file to load correct png
    ...
    def wind_speed(self):
        ...
        r = translate([50, 110, 3])(rotate([0, 0, 180])(
            linear_extrude(height=3, convexity=1)(text(f'Wind Speed: {self.windspeed} m/s',
                                                  size=5,
                                                  font="palatino",
                                                  halign="center",
                                                  valign="center"))
        ...
        cadfile = "out.scad"
        stlfile = "windspeed.stl"
        scad_render_to_file(r, cadfile)
        os.system("openscad {} -o {}".format(cadfile, stlfile))
        time.sleep(2)

    def mergeSTL(self):
        file_one = mesh.Mesh.from_file('./bottom.stl')
        file_two = mesh.Mesh.from_file('./symbol.stl')
        file_three = mesh.Mesh.from_file('./windspeed.stl')

        combined = mesh.Mesh(np.concatenate([file_one.data, file_two.data, file_three.data]))
        combined.save('yourWeather.stl', mode=stl.Mode.ASCII)

        print('STL file complete!')
        print('Check the project folder to find your file! \n Filename:yourWeather.stl')
